...
No secret keys or API keys are required. This project uses a fully local
Dockerized ClickHouse cluster. All connection parameters are in `config.py`
and `docker-compose.yml`. No `.env` file is needed.

> Note: `sleep 10` in the setup allows time for the ClickHouse containers
> to become healthy before running `schema.py`.

## Dataset

This project uses synthetic data generated by `generate_data.py`;
no separate dataset download is required. The script automatically
creates and inserts 1,000,000 rows into the ClickHouse cluster.

## Reproduce Results

Run the following commands in order to fully reproduce all results:

```bash
docker compose up -d
sleep 10
python3 schema.py        # Create database and tables
python3 generate_data.py # Generate and insert 1M rows
python3 app.py           # Run interactive CLI application
python3 plots.py         # Regenerate all figures (saved to plots/)
```
